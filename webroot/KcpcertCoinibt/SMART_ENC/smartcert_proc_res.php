...
if( $cert_enc_use == "Y" )
{
	if( $res_cd == "0000" )
	{
		...
		$user_birthday = date('Ymd',strtotime($birth_day));
		$today = date('Ymd');
		$age = floor(($today - $user_birthday) / 10000); // 유저의 만 나이
		if($age < 14){ // 만 14세 
			echo '<script>window.onload = function(){if( ( navigator.userAgent.indexOf("Android") > - 1 || navigator.userAgent.indexOf("iPhone") > - 1 || navigator.userAgent.indexOf("iPad") > - 1 )){window.parent.alert("만 14세 미만은 가입이 불가능합니다.");window.parent.location.reload();} else { alert("만 14세 미만은 가입이 불가능합니다."); window.close();  }}</script>';
			return;
		}
            ...
            window.onload=function()
            {
                try
                {
					if( ( navigator.userAgent.indexOf("Android") > - 1 || navigator.userAgent.indexOf("iPhone") > - 1 || navigator.userAgent.indexOf("iPad") > - 1 ) ) {
						...
						parent.document.signup.phone.value = "<?= f_get_parm_str( $phone_no ) ?>";
						parent.document.signup.user_name.value = "<?= f_get_parm_str( $user_name ) ?>";
						parent.document.querySelector(".container").style.display = '';
						parent.document.querySelector(".header2").style.display = '';
						parent.document.querySelector("#kcp_cert").style.display = 'none';
						parent.certification();

					} else {
						opener.document.signup.phone.value = "<?= f_get_parm_str( $phone_no ) ?>";
						opener.document.signup.user_name.value = "<?= f_get_parm_str( $user_name ) ?>";
