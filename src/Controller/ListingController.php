...
{ 
	...
	public function index()
	{
		...
		$findAllUsers = $this->Users->find("all",["fields"=>["id","eth_address"],"conditions"=>["eth_address IS NOT NULL"]])->hydrate(false)->toArray();
		$groupArr = [];
		$collectArr = [];
		$i=1;
		foreach($findAllUsers as $findSingleUser){
			$userAddr =$findSingleUser["eth_address"];
			$userId =$findSingleUser["id"];
			$curl = curl_init();

			curl_setopt_array($curl, array(
			  CURLOPT_URL => 'http://15.164.225.146:3000/get_balance?address='.$userAddr,
			...
			curl_close($curl);
			if($response>0){
				$collectArr[]=["id"=>$userId,"eth_address"=>$userAddr];
			}
			
			$i++;
		}
		print_r($collectArr); die; */
		die;
