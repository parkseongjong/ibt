...
use Cake\Event\Event;
	...
	class PagesController extends AppController
	{
		...
		public function dashboard(){ 
			...
			$getAllPair = $this->Coinpair->find('all',['conditions'=>['Coinpair.status'=>1],
	...
	public function withdrawal($coin=null){
		...
		if ($this->request->is(['post','put'])) {
				...
				if($withdrawalAmount<=0.02){
					$this->Flash->error('Amount should be grater than 0.02.');
					return $this->redirect(['controller'=>'pages','action'=>'withdrawal',$coin]);
				}	
				...
				if($coinType=="flat"){
					...
					if(!empty($tx_id)){
						...
						if($insertIntoTransactions){
							...
							$this->Flash->success($coin.' withdrawal request generated.');
							return $this->redirect(['controller'=>'pages','action'=>'withdrawal',$coin]);
						}
						else {
							$this->Flash->error($coin.' withdrawal failed, Try Again.');
							return $this->redirect(['controller'=>'pages','action'=>'withdrawal',$coin]);
						}
					}
					else {
	...
	public function ramethwithdrawal(){
		...
		if ($this->request->is(['post','put'])) {
				...
				if(!in_array($amountInUsd,$ramUsdArr)){
					$this->Flash->error('Amount should be 100 or 200.');
					return $this->redirect(['controller'=>'pages','action'=>'ramethwithdrawal']);
				}
				...
				if(!empty($tx_id)){
					...
					if($insertIntoTransactions){
						...
						$this->Flash->success($coin.' withdrawal successfully. Transactions ID : '.$tx_id);
						return $this->redirect(['controller'=>'pages','action'=>'ramethwithdrawal']);
					}
					else {
						$this->Flash->error($coin.' withdrawal failed, Try Again.');
		...
		/*===================================================*/
		  ...
		  if ($this->request->is(['post','put'])) {
				...
				if(empty($getSelectAmount)){
					$this->Flash->error('Please Select Amount.');
					return $this->redirect(['controller'=>'pages','action'=>
					'usdwithdrawalamo']);
				 }
				$getSelectType = $this->request->data['amount_in_usd'];
				  if($getSelectType==2){
						...
						if (!in_array($getSelectAmount,$selectAmountEth))  {
							$this->Flash->error('Please Select a correct eth amount.');
		  ...
		  if ($this->request->is(['post','put'])) {
				...
				$getSelectType = $this->request->data['amount_in_usd'];
				  if($getSelectType==2){
						...
						if (!in_array($getSelectAmount,$selectAmountEth))  {
							$this->Flash->error('Please Select a correct ram amount.');
							return $this->redirect(['controller'=>'pages','action'=>'worldwithdrawal']);
						}
				   }
						...
						$selectAmountEth=array(20);
						if (!in_array($getSelectAmount,$selectAmountEth))  {
							$this->Flash->error('Please Select a correct admc amount.');
							return $this->redirect(['controller'=>'pages','action'=>'worldwithdrawal']);
						}
				   }
		...
		if ($this->request->is(['post','put'])) {
				...
				if(!empty($tx_id)){
					...
					if($insertIntoTransactions){
						...
						$this->Flash->success($coin.' withdrawal successfully. Transaction ID : '.$tx_id);
						return $this->redirect(['controller'=>'pages','action'=>'ramwithdrawal',$coin]);
					}
					else {
						$this->Flash->error($coin.' withdrawal failed, Try Again.');
						...
						$this->Flash->error($coin.' withdrawal failed, Try Againss.');
						return $this->redirect(['controller'=>'pages','action'=>'ramwithdrawal',$coin]);
				}
		}
				...
				if(!empty($callRamtrexApi)){
					...
					if($returnData['Status']=="success"){
						...
						if($insertIntoTransactions){
							...
							$this->Flash->success($coin.' deposit successfully.');
							return $this->redirect(['controller'=>'pages','action'=>'ramdeposit']);
						}
						else {
							$this->Flash->error('Unable to deposit Ram Token');
							return $this->redirect(['controller'=>'pages','action'=>'ramdeposit']);
						}
					}
					else {
	...
	public function flatExchange(){
		...
		if ($this->request->is(['post','put'])) {
			...
			if(empty($findCoinDetail)){
				$this->Flash->error('Please enter valid Currency.');
				return $this->redirect(['controller'=>'pages','action'=>'flatExchange']);
			}
			...
			if($insertIntoTransactions){
				...
				$this->Flash->success('Exchange With NTR Successfully.');
				return $this->redirect(['controller'=>'pages','action'=>'flatExchange']);
			}
