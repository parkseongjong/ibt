...
use Cake\Event\Event;
	...
	class PagesController extends AppController
	{
		...
		public function dashboard(){ 
			...
			// total BTC Amount 
			$totalBtcAmt = $this->Transactions->find('all',['fields'=>["coin_amt_sum"=>'sum(coin_amount)'],
															'conditions'=>['cryptocoin_id'=>1,
																		   'tx_type'=>'purchase',
																		   'status'=>'completed']])
																		   ->hydrate(false)
																		   ->toArray();
			$totalBtcAmt = $totalBtcAmt[0]['coin_amt_sum'];
			$this->set('totalBtcAmt',$totalBtcAmt);
			
			// total Eth Amount 
			$totalEthAmt = $this->Transactions->find('all',['fields'=>["coin_amt_sum"=>'sum(coin_amount)'],
															'conditions'=>['cryptocoin_id'=>2,
																		   'tx_type'=>'purchase',
																		   'status'=>'completed']])
																		   ->hydrate(false)
																		   ->toArray();
			$totalEthAmt = $totalEthAmt[0]['coin_amt_sum'];
			$this->set('totalEthAmt',$totalEthAmt);	
