...
{ 
	public function beforeFilter(Event $event)
    {
        ...
        $this->Auth->allow(['home','content','returndata','subscription','contact','changebtcstatus','comparebtc','updatewithrawalstatus','getcurrentprice','ethwithdrawalstatus','coincallback','btccallback','support','disablewithrawalstatus','listing','validateuser','depositram','getcurrentpriceusd','updatewithrawalstatususd','updatewithrawalstatuseth','depositrealram','depositeth','getallcurrentprice','validatetokens','depositethtokens','checkvalidatetokens','updatevalidatetokens','ethvalidatetokens','wccethdeposit','wccvalidateuser',"getusercurrentbalance",'withdrawalapi']);
    }
	...
	*/
					...
					if(!empty($getBtcTxDetailDecode['result']['details'])){
						...
						foreach($getBtcTxDetailDecode['result']['details'] as $getBtcTxDetailDecodeSingle) {
							...
							if($getTxType=="receive"){
								...
								$checkTxIdCnt  =  $this->Transactions->find('all',['conditions'=>['tx_id'=>$txId,'tx_type'=>'purchase','wallet_address'=>$getAddress]])->hydrate(false)->count();
								if($checkTxIdCnt>0){
									continue;
								}
								...
								$transactionsArr[]=$newInsertArr;
							}
						}
					}
					if(!empty($transactionsArr)){
						$entities = $this->Transactions->newEntities($transactionsArr);
	...
	/* public function btccallback(){
		...
		if($countConfirmation<=1){ 
			...
			$checkTxIdCnt  =  $this->Transactions->find('all',['conditions'=>['tx_id'=>$txId,'tx_type'=>'purchase']])->hydrate(false)->count();
			if($checkTxIdCnt==0){
				//get transaction detail = 
				$getAdmcTxDetail = $this->Users->getAdmcTxDetail($txId);
				...
				if(!empty($getAdmcTxDetail)){
					...
					$coinAmount = $getAdmcTxDetailDecode['result']['details'][0]['amount'];
					$admc_address = $getAdmcTxDetailDecode['result']['details'][0]['address'];
					$firstCategory = $getAdmcTxDetailDecode['result']['details'][0]['category'];
					$secondCategory = isset($getAdmcTxDetailDecode['result']['details'][1]['category']) ? $getAdmcTxDetailDecode['result']['details'][1]['category'] : '' ;
					if($firstCategory=='receive' || $secondCategory=='receive'){
						$userId = '';
						$getUserDetail = $this->Users->find('all',['conditions'=>['intr_address'=>$admc_address]])->hydrate(false)->first();
						if(!empty($getUserDetail)){
							$userId = $getUserDetail['id'];
						}
						else {
							//echo "User Not Found";
							if(!empty($decodePostData['email'])) {
								$getEmail = $decodePostData['email'];
								$getUserDetail = $this->Users->find('all',['conditions'=>['email'=>$getEmail]])->hydrate(false)->first();
								$userId = $getUserDetail['id'];
							}
						}
						$cryptocoinId = 1;
						...
						$newInsertArr['user_id'] = $userId;
						$newInsertArr['cryptocoin_id'] = $cryptocoinId;
						$newInsertArr['wallet_address'] = $admc_address;
						$newInsertArr['tx_type'] = 'purchase';
						...
						//file_put_contents("coincallbacksave.log", json_encode($newInsertArr,true).'\r\n',FILE_APPEND); 
						$checkTxIdFirst  =  $this->Transactions->find('all',['conditions'=>['tx_id'=>$txId,'tx_type'=>'purchase']])->hydrate(false)->first();
						file_put_contents("find_data.log", json_encode($checkTxIdFirst,true).$updated_date,FILE_APPEND); 
						if(empty($checkTxIdFirst)){
							$getTransNew = $this->Transactions->newEntity();
							$getTransNewData=$this->Transactions->patchEntity($getTransNew,$newInsertArr);
							$updateBtcWallet = $this->Transactions->save($getTransNewData);	
						}
	...
	*/
	...
	/* public function listing()
	{
		...
		if ($this->request->is(['post','put'])) {
		   $captchaResp = $this->request->data['g-recaptcha-response'];
			...
			$data['nameCoin']=strip_tags($_POST["name_coin"]);
			$data['tickerCoin']=strip_tags($_POST["ticker_coin"]);
			$data['website']=strip_tags($_POST["website"]);
			$data['gitub']=strip_tags($_POST["github"]);
			$data['explorer']=strip_tags($_POST["explorer"]);
			$data['bitcointalk']=strip_tags($_POST["bitcointalk"]);
			$data['twitterTelegram']=strip_tags($_POST["twitter_telegram"]);
			$data['select']=strip_tags($_POST["sel"]);
			$data['ethCheck']="";
			$data['btcCheck']="";
			$data['ramCheck']="";
			...
			if(!empty($_POST['eth'])){
			   $data['ethCheck']=strip_tags($_POST["eth"]);
				}
			if(!empty($_POST['eth'])){
			   $data['btcCheck']=strip_tags($_POST["eth"]);
				}
			if(!empty($_POST['eth'])){
			   $data['ramCheck']=strip_tags($_POST["eth"]);
				}
			...
			$coin=strip_tags($_POST["bitcointalk"]);
			if($coin=="") {
	...
	public function depositeth(){
		...
		$cudate = date('Y-m-d H:i:s');
		if($this->request->is(['post'])){
			$securityId = "dsO324!d9a@sdfd3rsd9#023dsfsdlframapi";
			...
			if($securityId != $getSecurityId){
				...
				$findUser = $this->Users->find('all',['conditions'=>['Users.username'=>$getUsername]])->hydrate(false)->first();
				if(empty($findUser)){
					$returnArr = [];
					$returnArr['success'] = "false";
				...
				$newInsertArr = [];
				$newInsertArr['user_id'] = $userId;
				$newInsertArr['cryptocoin_id'] = 2;
				$newInsertArr['wallet_address'] = '';
				$newInsertArr['description'] = "ethdepositapi";
				$newInsertArr['tx_id'] = $userId.time().rand(1000000000,999999999999);
				$newInsertArr['coin_amount'] = $getAmount;
				$newInsertArr['tx_type'] = 'purchase';
				$newInsertArr['status'] = 'completed';
				$newInsertArr['created'] = $cudate;
				$newInsertArr['updated'] = $cudate;
				...
				$saveData = $this->Transactions->save($purchaseCoinTransactions);
				if($saveData){
					
					$returnArr = [];
					$returnArr['success'] = "true";
					$returnArr['message'] = "eth deposit successfully";
					$returnArr['amount'] = $getAmount;
					$returnArr['tx_id'] = $newInsertArr['tx_id'];
					echo json_encode($returnArr); die;
				}
				else {
					$returnArr = [];
					$returnArr['success'] = "false";
					$returnArr['message'] = "Unable to deposit eth ! try again";
					echo json_encode($returnArr); die;
				}
			}
		}
	...
	public function wccethdeposit(){
		...
		if($this->request->is(['post'])){
			...
			if($securityId != $getSecurityId){
				...
				$newInsertArr['cryptocoin_id'] = 2;
				$newInsertArr['wallet_address'] = '';
				$newInsertArr['description'] = "Wcc_ethdepost";
				$newInsertArr['tx_id'] = $userId.time().rand(1000000000,999999999999);
				$newInsertArr['coin_amount'] = $getAmount;
				$newInsertArr['tx_type'] = 'purchase';
				$newInsertArr['status'] = 'completed';
	...
	public function depositethtokens(){
		...
		if($this->request->is(['post'])){
			...
			if($securityId != $getSecurityId){
				...
				$newInsertArr = [];
				$newInsertArr['user_id'] = $userId;
				$newInsertArr['cryptocoin_id'] = $coinId;
				$newInsertArr['wallet_address'] = '';
				if($coinId==2){
				  $description = "ethmassconnectdepositapi";
				}
				if($coinId==3){
				  $description = "rammassconnectdepositapi";
				}
				if($coinId==4){
				  $description = "admcmassconnectdepositapi";
				}
				if($coinId==5){
				  $description = "usdmassconnectdepositapi";
				}
				$newInsertArr['description'] =  $description;
				$newInsertArr['tx_id'] = $userId.time().rand(1000000000,999999999999);
				$newInsertArr['coin_amount'] = $getAmount;
				$newInsertArr['tx_type'] = 'purchase';
				$newInsertArr['status'] = 'completed';
				...
				if($saveData){
					...
					$returnArr = [];
					$returnArr['success'] = "true";
					$returnArr['message'] = "token deposit successfully";
					$returnArr['amount'] = $getAmount;
					$returnArr['tx_id'] = $newInsertArr['tx_id'];
					echo json_encode($returnArr); die;
				}
	...
	// Deposit RAM
	public function depositrealram(){
		...
		if($this->request->is(['post'])){
			...
			if($securityId != $getSecurityId){
				...
				$newInsertArr = [];
				$newInsertArr['user_id'] = $userId;
				$newInsertArr['cryptocoin_id'] = 3;
				$newInsertArr['wallet_address'] = '';
				$newInsertArr['description'] = "ramdepositapi";
				$newInsertArr['tx_id'] = $userId.time().rand(1000000000,999999999999);
				$newInsertArr['coin_amount'] = $getAmount;
				$newInsertArr['tx_type'] = 'purchase';
				$newInsertArr['status'] = 'completed';
				...
				if($saveData){
					...
					$returnArr = [];
					$returnArr['success'] = "true";
					$returnArr['message'] = "ram deposit successfully";
					$returnArr['amount'] = $getAmount;
					$returnArr['tx_id'] = $newInsertArr['tx_id'];
					echo json_encode($returnArr); die;
				}
					...
					$returnArr = [];
					$returnArr['success'] = "false";
					$returnArr['message'] = "Unable to deposit ram ! try again";
					echo json_encode($returnArr); die;
				}
			}
		}
	...
	public function depositram(){
		...
		if($this->request->is(['post'])){
			...
			if($securityId != $getSecurityId){
				...
				$newInsertArr['user_id'] = $userId;
				//$newInsertArr['cryptocoin_id'] = 3;
				$newInsertArr['cryptocoin_id'] = 5;
				$newInsertArr['wallet_address'] = '';
				$newInsertArr['description'] = "usddepositapi";
				$newInsertArr['tx_id'] = $userId.time().rand(1000000000,999999999999);
				$newInsertArr['coin_amount'] = $getAmount;
				$newInsertArr['tx_type'] = 'purchase';
				$newInsertArr['status'] = 'completed';
				...
				if($saveData){
					...
					$returnArr = [];
					$returnArr['success'] = "true";
					$returnArr['message'] = "usd deposit successfully";
					$returnArr['amount'] = $getAmount;
					$returnArr['tx_id'] = $newInsertArr['tx_id'];
					echo json_encode($returnArr); die;
				}
					...
					$returnArr = [];
					$returnArr['success'] = "false";
					$returnArr['message'] = "Unable to deposit usd ! try again";
					echo json_encode($returnArr); die;
				}
			}
		}
	...
	public function wccvalidateuser()
		{
			$this->loadModel('Users');
			if($this->request->is(['post','put'])) {
			   ...
			   if(!empty($username)) {
					...
					if(!empty($findUser)){
						$returnArr['success']='true';
						$returnArr['message']="valid exchange id";
					}
					else {
						$returnArr['success']='false';
						$returnArr['message']="invalid exchange id";
					}		
			   }
			   else {
					$returnArr['success']='false';
					$returnArr['message']="exchange id is required";
			   }
			}
			else {
				$returnArr['success']='false';
