<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1> <?=$display_type?> <small>Requests</small> </h1>
        <ol class="breadcrumb">
            <li><a href="<?php echo $this->Url->build(['controller'=>'pages','action'=>'dashboard']);  ?>"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active"><?=$display_type?> Requests</li>
        </ol>
    </section>
                    ...
                    <table id="table-two-axis" class="two-axis table">
                       ...
                       // pr($data);die;
                        ...
                        <?php  if(count($listing->toArray()) < 1) {
                            echo "<tr class='odd'><td colspan = '6'>No record found</td></tr>";
                        } ?>
                        </tbody>
                    </table>
                    <?php $this->Paginator->options(array('url' => array('controller' => 'Transactions', 'action' => 'buy',$type)));
...
function request_check(val,id) {
	if(confirm("Are you sure?"))
	{
		$.ajax({
			type : 'POST',
			url:'<?php echo $this->Url->build(['controller'=>'transactions','action'=>'request_update']); ?>',
			data:{'val':val,'id':id},
			dataType : 'JSON',
