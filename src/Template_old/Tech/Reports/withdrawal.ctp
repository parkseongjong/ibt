		...
		<div class="clearfix"></div>
                   ...
                   <div class="w3l-table-info agile_info_shadow">
                    ...
                    <table id="table-two-axis" class="two-axis table">
						...
						$usedStatus = ($data['withdrawal_send']=='N') ? "<div style='cursor:pointer;color:blue;' id='status_id_".$data['id']."' onClick='change_withdrawal_status(".$data['id'].")'>Not Used</div>" : "Used" ;
...
$(document).ready(function() {
	...
	function change_withdrawal_status(id){
		...
		bootbox.confirm(ques, function(result) {
			if(result == true){
				jQuery.ajax({ 
					url: '<?php echo $this->Url->build(['controller'=>'Reports','action'=>'withdrawalUpdate']); ?>',
					data: {'id':id,'status':"Y"},
					type: 'POST',
					success: function(data) {
						if(data == 1){
