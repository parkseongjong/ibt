<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1> Admc Withdrawal <small>Listing</small> </h1>
        <ol class="breadcrumb">
            <li><a href="<?php echo $this->Url->build(['controller'=>'pages','action'=>'dashboard']);  ?>"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active"> Admc Withdrawal</li>
        </ol>
    </section>
    ...
    <section id="content" class="table-layout">
		...
		<div class="clearfix"></div>
                    ...
                    <h3 class="w3_inner_tittle two">Admc withdrawal list</h3>
                   <div class="w3l-table-info agile_info_shadow">
...
$(document).ready(function() {
	...
	function change_withdrawal_status(id){
		...
		bootbox.confirm(ques, function(result) {
			if(result == true){
				jQuery.ajax({ 
					url: '<?php echo $this->Url->build(['controller'=>'Reports','action'=>'admcWithdrawalUpdate']); ?>',
					data: {'id':id,'status':"Y"},
					type: 'POST',
					success: function(data) {
						if(data == 1){
							jQuery("#status_id_"+id).html('Completed').css('color','#000;').attr('onClick','#');
							new PNotify({
								  title: 'Success',
