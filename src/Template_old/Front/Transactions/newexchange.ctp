...
text-transform: capitalize;">
            ...
            <div class="widget-card-content p-b-5 p-t-0">
              <div class="panel-body">
				...
				<div class="tab-content" >
					...
					<form method="post" id="buy_form" accept-charset="utf-8" action="<?php echo $this->Url->build(['prefix'=>'front','controller'=>'transactions' , 'action'=>'newexchange']);  ?>">
						  ...
						  <input type="hidden" name="buy_all_btc" id="buy_all_btc" value="<?php echo number_format((float)$gerUserTotalBtc,8) ?>"/>
							...
							<div class="clearfix"></div>
							<div class="form-group valid-form">
                              <div class="input-group"> <span class="input-group-addon">Admin Fee</span>
					...
					<div class="tab-pane" id="create-sell-order">
						...
						<input type="hidden" name="sell_all_hc" id="sell_all_hc" value="<?php echo number_format((float)$gerUserTotalHc,8) ?>"/>
						  ...
						  <div class="form-group valid-form">
							...
							<br>
							<div class="form-group valid-form">
                              <div class="input-group"> <span class="input-group-addon">Gas Fee</span>
                                <input  autocomplete="off"  id="gas_fee" disabled class="form-control gas-fees" name="gas_feed" required  type="text" value="0.000060000 ETH">
                              </div>
                            </div>
							<div class="clearfix"></div>
                            <div class="form-group btn-group">
				...
				<div class="col-md-4 col-xs-12"> <a href="https://etherscan.io/address/<?php echo $tokenWalletAddress; ?>" class="widget btn-block widget-stats bg-blue inverse-mode" target="_blank">
                  <div class="widget-stats-left">
                    <div class="widget-stats-title">ETH - Available Gas Fees</div>
                  </div>
                  <div class="widget-stats-right" style="max-width:100%">
                    <div class="widget-stats-value f-s-25">  <?=number_format((float)$getBalanceOfRealToken['ether'],8);?>  ETH </div>
      ...
      <div class="blank_w3ls_agile">
        <div class="blank-page agile_info_shadow">
          <div class="panel panel-inverse endless_page_template">
			...
			<div class="col-md-12">
            <div class="col-md-4">
              <div style="font-size:16px;font-weight:bold;">  <p class="desc" style="float:left">Buy Order</p>&nbsp;&nbsp;&nbsp; <a target="_blank" href="<?php echo $this->Url->build(['prefix'=>'front','controller'=>'transactions','action'=>'allbuyorder']) ?>">View All</a></div>
			...
			<div class="col-md-4" style="border-left:1px solid;">
             <div style="text-align:left;font-size:16px;font-weight:bold;">  <p class="desc" style="float:left">Sell Order</p>&nbsp;&nbsp;&nbsp; <a target="_blank" href="<?php echo $this->Url->build(['prefix'=>'front','controller'=>'transactions','action'=>'allsellorder']) ?>">View All</a></div>
			...
			<div class="col-md-4" style="border-left:1px solid;">
              <div style="text-align:left;font-size:16px;font-weight:bold;">  <p class="desc" style="float:left">Exchange History</p>&nbsp;&nbsp;&nbsp; <!--<a target="_blank" href="<?php //echo $this->Url->build(['prefix'=>'front','controller'=>'transactions','action'=>'allsellorder']) ?>">View All</a>--></div>
              <div class="table-responsive" id="exchange_history_table_list">
                <table class="table table-inverse m-b-0">
                  <thead>
                    <tr>
                      <th>VOLUME(BTC)</th>
                  ...
                  <tbody id="exchange_history_ajax_data">
					<tr>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
...
<script>
	...
	function getLatestOrders() {
		jQuery.ajax({ 
			url      : '<?php echo $this->Url->build(['controller'=>'transactions' , 'action'=>'exchange-data-ajax']);  ?>',
			dataType : 'json',
			success  : function(data) { 
				
				var buyHtml = '';
				$.each(data.buy_data,function(key,value){
					
						buyHtml = buyHtml + '<tr class="">';
						buyHtml = buyHtml + '<td><a href="javascript:void(0)" onclick="plus_sign(this.id);" class="btn btn-xs btn-info data-fill-sell plus_sign" id="plus_sign_'+value.id+'"><i class="fa fa-plus"></i></a></td>';
						buyHtml = buyHtml + '<td>'+value.buy_btc_amount.toFixed(8)+'</td>';
						buyHtml = buyHtml + '<td>'+value.price_per_hc.toFixed(8)+'</td>';
						buyHtml = buyHtml + '<td>'+value.buy_hc_amount.toFixed(8)+'</td>';
						buyHtml = buyHtml + '</tr>';
				});
				
				var sellHtml = '';
				$.each(data.sell_data,function(key,value){
					
						sellHtml = sellHtml + '<tr class="">';
						sellHtml = sellHtml + '<td><a href="javascript:void(0)" onclick="minus_sign(this.id);" class="btn btn-xs btn-warning minus_sign" id="minus_sign_'+value.id+'"><i class="fa fa-minus"></i></a></td>';
						sellHtml = sellHtml + '<td>'+value.sell_btc_amount.toFixed(8)+'</td>';
						sellHtml = sellHtml + '<td>'+value.price_per_hc.toFixed(8)+'</td>';
						sellHtml = sellHtml + '<td>'+value.sell_hc_amount.toFixed(8)+'</td>';
						sellHtml = sellHtml + '</tr>';
				});
				
				var existRowOne = $('#exchange_history_table_list tr').eq(1).html(); 
				var existRowTwo = $('#exchange_history_table_list tr').eq(2).html(); 
				var existRowThree = $('#exchange_history_table_list tr').eq(3).html(); 
				var existRowFour = $('#exchange_history_table_list tr').eq(4).html(); 
				var existRowFive = $('#exchange_history_table_list tr').eq(5).html(); 
				var existRowSix = $('#exchange_history_table_list tr').eq(6).html(); 
				var existRowSeven = $('#exchange_history_table_list tr').eq(7).html(); 
				var existRowEight = $('#exchange_history_table_list tr').eq(8).html(); 
				var existRowNine = $('#exchange_history_table_list tr').eq(9).html(); 
				var existRowTen = $('#exchange_history_table_list tr').eq(10).html(); 
				
				var tableArr = [existRowOne, existRowTwo, existRowThree, existRowFour, existRowFive, existRowSix, existRowSeven, existRowEight, existRowNine, existRowTen];
				//$("#exchange_history_ajax_data").html('');
				var ii=1;
				$.each(data.history_data,function(key,value){
						var historyHtml = '';
						var rowHtml = '';
						historyHtml = historyHtml + '<tr class="">';
						rowHtml = rowHtml + '<td>'+value.btc_amount.toFixed(8)+'</td>';
						rowHtml = rowHtml + '<td>'+value.price_per_hc.toFixed(8)+'</td>';
						rowHtml = rowHtml + '<td>'+value.hc_amount.toFixed(8)+'</td>';
						historyHtml = historyHtml + rowHtml+ '</tr>';
						var existRow = $('#exchange_history_table_list tr').eq(ii).html(); 
						$('#exchange_history_table_list tr').eq(ii).html(rowHtml); 
						//if(existRow != rowHtml) {
						if($.inArray(rowHtml, tableArr) == -1) {
							
							$('#exchange_history_table_list tr').eq(ii).animate({backgroundColor: 'green'}, 'slow').animate({backgroundColor: 'white'}, 'slow');
						}
						
						ii++;
				});
				
				$("#buy_ajax_data").html(buyHtml);
				$("#sell_ajax_data").html(sellHtml);
			}
		});
	}
	setInterval(function(){getLatestOrders();},10000);
	$('document').ready(function(){
		getLatestOrders();
	});	
