...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1> <a href="<?php echo $this->Url->build(['controller'=>'reports','action'=>'multisignwithdrawalmain']);  ?>"> <?= __('Multisig Main Account Withdrawals');?></a> </h1>
        <ol class="breadcrumb">
            <li><a href="<?php echo $this->Url->build(['controller'=>'pages','action'=>'dashboard']);  ?>"><i class="fa fa-dashboard"></i> <?= __('Home');?></a></li>
            <li class="active"><a href="<?php echo $this->Url->build(['controller'=>'reports','action'=>'multisignwithdrawalmain']);  ?>"> <?= __('Multisig Main Account Withdrawals');?></a></li>
        </ol>
    </section>
    <!-- Main content -->
            ...
            <div class="agile-tables">
                <div class="w3l-table-info agile_info_shadow">
                    ...
                    <?php
						...
						$this->Paginator->options(array('url' => array('controller' => 'Reports', 'action' => 'multisignwithdrawalmain')+$searchArr));
...
function signTranactionsClick(getIndexId){
	jQuery("#conf_yes").attr('data-attr',getIndexId);
	jQuery("#myModalConfirm").modal('show');
}
function signTranactions(getIndexId){
		jQuery.ajax({ 
			url: '<?php echo $this->Url->build(['controller'=>'Reports','action'=>'signwithdrawalmain']); ?>',
    ...
    $(document).ready(function() {
		...
		$("#model_form").submit(function(){ 
			...
			if(id!='') {
				jQuery.ajax({ 
					url: '<?php echo $this->Url->build(['controller'=>'Reports','action'=>'ethWithdrawalUpdateMain']); ?>',
					beforeSend: function(xhr){
						xhr.setRequestHeader('X-CSRF-Token', $("input[name='_csrfToken']").val());
					},
					data: {'id':id,
