<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1> Ram Deposit <small>Listing</small> </h1>
        <ol class="breadcrumb">
            <li><a href="<?php echo $this->Url->build(['controller'=>'pages','action'=>'dashboard']);  ?>"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">Ram Deposit</li>
        </ol>
    </section>
    ...
    <section id="content" class="table-layout">
        ...
        <div class="inner_content_w3_agile_info">
            ...
            <form style="padding:10px" method="post" class="form-horizontal form-label-left input_mask">
                     ...
                     <div class="form-group">
						...
						<div class="col-md-3 col-sm-3 col-xs-12">
                            <?php  echo $this->Form->input('withdrawal_tx_id',array('placeholder'=>'Tx Id','class' => 'form-control col-md-7 col-xs-12','label' =>false,'type'=>'text')); ?>
                        </div>
		...
		<div class="clearfix"></div>
                    ...
                    <h3 class="w3_inner_tittle two">RAM Deposit list</h3>
                   <div class="w3l-table-info agile_info_shadow">
                    ...
                    <?php $this->Paginator->options(array('url' => array('controller' => 'Reports', 'action' => 'ramDepositSearch')));
...
$(document).ready(function() {
	...
	function change_withdrawal_status(id){
		...
		bootbox.confirm(ques, function(result) {
			if(result == true){
				jQuery.ajax({ 
					url: '<?php echo $this->Url->build(['controller'=>'Reports','action'=>'ramWithdrawalUpdate']); ?>',
					data: {'id':id,'status':"Y"},
					type: 'POST',
					success: function(data) {
						if(data == 1){
	...
	function change_withdrawal_status_used(id){
		...
		bootbox.confirm(ques, function(result) {
			if(result == true){
				jQuery.ajax({ 
					url: '<?php echo $this->Url->build(['controller'=>'Reports','action'=>'ramWithdrawalUpdateNotUsed']); ?>',
					data: {'id':id,'status':"N"},
					type: 'POST',
					success: function(data) {
						if(data == 1){
