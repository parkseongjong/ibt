<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1> <a href="<?php echo $this->Url->build(['controller'=>'transactions','action'=>'depositlist']);?>"><?=__("Users Deposit Amount List"); ?></a></h1>
        <ol class="breadcrumb">
            <li><a href="<?php echo $this->Url->build(['controller'=>'pages','action'=>'dashboard']);  ?>"><i class="fa fa-dashboard"></i><?=__("Home");?></a></li>
            <li class="active"> <a href="<?php echo $this->Url->build(['controller'=>'transactions','action'=>'depositlist']);  ?>"><?= __("Users Deposit Amount List"); ?></a></li>
        </ol>
    </section>
    <!-- Main content -->
                    ...
                    <form id="frm" method="get" >
                        <div class="form-group">
                            ...
                            <div class="col-md-4 col-sm-3 col-xs-12">
                                <?php  echo $this->Form->input('',array('id'=>'total_amount','placeholder'=>__('Total Amount Deposited: '),'class' => 'form-control col-md-7 col-xs-12 has-feedback-left','label' =>false,'type'=>'text','readonly'=>'readonly','value'=>number_format($totalDepositAmount) . ' KRW')); ?>
                            </div>
                    ...
                    <div id="transferHistory" class="mt10 table-responsive">
                        <table class="two-axis table" id="historyData">
                            ...
                            foreach($listing->toArray() as $k=>$data){
								$this->add_system_log(200, $data['user_id'], 1, '고객 KRW 입금 목록 조회 (이름, 번호)');
                                if($k%2==0) $class="odd";
                                else $class="even";
                                ?>
                                <tr class="<?=$class?>">
...
<!--                                        <a href="javascript:void(0)" id="del_status_id_--><?//= $data['id']; ?><!--" onclick="change_del_status(this,<?php echo $data['id'] ?>)">-->
                            ...
                            <?php  if(count($listing->toArray()) < 1) {
                                 echo '<tr class="even"><td colspan = "12" style="text-align: center;">'.__("No record found").'</td></tr>';
                            } ?>
                            </tbody>
                        </table>
                        <?php
							...
							$this->Paginator->options(array('url' => array('controller' => 'transactions', 'action' => 'depositlist')+$searchArr));
...
<script>
    ...
    function getUserInfo(id){
        ...
        $.ajax({
            url: "<?php echo $this->Url->build(['controller'=>'transactions','action'=>'depositlistajax']); ?>/",
            type:'POST',
            data: {id:id},
            dataType: 'JSON',
    ...
    function getUserNameInfo(userId){
        $("tbody").html('<tr id="ajax_coin_tr" style="vertical-align: center;alignment: center;"><td colspan="2">&nbsp;</td><td ><img id="model_qr_code_flat" src="/ajax-loader.gif" /></td><td colspan="2">&nbsp;</td></tr>');
        $.ajax({
            url: "<?php echo $this->Url->build(['controller'=>'transactions','action'=>'depositlistajaxname']); ?>/",
            data: {user_id:userId},
            type:'POST',
            dataType: 'JSON',
            success: function(resp) {
                ...
                $("#transferHistory").hide();
                $("#search_users").hide();
                let depo =  resp.data.totalDepositAmount;
                $('#total_amount').val("Total Deposited Amount: " + numberWithCommas(depo) + " KRW");
            },
            error: function (e) {
                $("#ajax_coin_tr").hide();
                //$("#model_qr_code_flat").hide();
    ...
    $(document).ready(function(){
        //$("#transferList").hide();
        $("#list_no").select2();
		list_select2('list_no','depositlist');
...
//		});
    ...
    function change_del_status_fix(id,type){
        ...
        if(status === 'deleted'){
            ...
            var ques= "Do you want to Delete?";
            var status = "deleted";
    ...
    /*function change_del_status(getThis,id){
        var status = $(getThis).prev().val();
        //var status = $("#user_del_status_"+id).val();
        if(status === 'deleted'){
            var status = "deleted";
            var change = '<button type="button" class="btn btn-danger btn-xs" disabled>Deleted</button>'
        }else{
            var ques= "Do you want to Delete?";
            var status = "deleted";
            var change = '<button type="button" class="btn btn-success btn-xs" disabled>Deleted</button>';
        }
                ...
                jQuery.ajax({
                    ...
                    success: function(data) {
                        if(data == 1){
                            jQuery(getThis).html(change);
                            jQuery(getThis).prev().val(status);
                            // jQuery("#del_status_id_"+id).html(change);
                            jQuery("#status_id_"+id).html(change);
                            // jQuery("#user_del_status_"+id).val(status);
                            new PNotify({
                                title: 'Success',
