<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1> <a href="<?php echo $this->Url->build(['controller'=>'transactions','action'=>'couponslist']);  ?>"><?=__("Users Coupons List");?></a></h1>
        <ol class="breadcrumb">
            <li><a href="<?php echo $this->Url->build(['controller'=>'pages','action'=>'dashboard']);  ?>"><i class="fa fa-dashboard"></i><?=__("Home");?></a></li>
            <li class="active"> <a href="<?php echo $this->Url->build(['controller'=>'transactions','action'=>'couponslist']);  ?>"><?=__("Users Coupons List");?></a></li>
        </ol>
    </section>
    <!-- Main content -->
                    ...
                    <div id="transferHistory" class="mt10 table-responsive">
                        <table class="two-axis table" id="historyData">
                            ...
                            foreach($listing->toArray() as $k=>$data){
								$this->add_system_log(200, $data['user_id'], 1, '고객 쿠폰 목록 조회 (이름, 전화번호, 계좌번호)');
                                if($k%2==0) $class="odd";
                                else $class="even";
                            ...
                            <?php  if(count($listing->toArray()) < 1) {
                                 echo '<tr class="even"><td colspan = "11" style="text-align: center;">'.__("No record found").'</td></tr>';
                            } ?>
                            </tbody>
                        </table>
                        <?php
							...
							$this->Paginator->options(array('url' => array('controller' => 'transactions', 'action' => 'couponslist')+$searchArr));
...
<script>
    function getUserInfo(id){
        ...
        $.ajax({
            url: "<?php echo $this->Url->build(['controller'=>'transactions','action'=>'couponslistajax']); ?>/",
            type:'POST',
            data: {id:id},
            dataType: 'JSON',
            success: function(resp) {
                if (resp.success === "false") {
                    ...
                    if(getData.tx_type === "bought_coupon"){
                        html = html + '<td>Bought Coupon</td>';
                    }
    ...
    function getUserNameInfo(userId){
        $("tbody").html('<tr id="ajax_coin_tr" style="vertical-align: center;alignment: center;"><td colspan="2">&nbsp;</td><td ><img id="model_qr_code_flat" src="/ajax-loader.gif" /></td><td colspan="2">&nbsp;</td></tr>');
        $.ajax({
            url: "<?php echo $this->Url->build(['controller'=>'transactions','action'=>'couponslistajaxname']); ?>/",
            data: {user_id:userId},
            type:'POST',
            dataType: 'JSON',
            success: function(resp) {
    ...
    $(document).ready(function(){
        ...
        list_select2('list_no','couponslist');
