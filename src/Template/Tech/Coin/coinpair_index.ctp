                    ...
                    <div class="agile-validation agile_info_shadow">
                        <div class="validation-grids widget-shadow" data-example-id="basic-forms">
                            <div class="input-info">
                                <h3 class="w3_inner_tittle two"><?= __('List of Coin Pairs'); ?></h3>
								<a class="btn btn-info" href="<?php echo $this->Url->build(['controller'=>'Coin','action'=>'coinpairAdd']) ?>"><?= __('Add new coin pair'); ?></a>
                            </div>
							 <?= $this->Flash->render() ?>
							...
							<thead>
								<tr>
									...
									<th><?= __('Order No.'); ?></th>
									<th><?= __('First Coin'); ?></th>
									<th><?= __('Second Coin'); ?></th>
									<th><?= __('Max Min Price %'); ?></th>
									<th><?= __('Status'); ?></th>
							...
							<tr style="text-align:center;" class="<?=$class?>" id="user_row_<?= $data['id']; ?>">
								<td> <?=$count?></td>
								<td class="order_no_<?= $data['id']; ?>" ><?php echo $data['order_no']; ?> </td>
								<td class="total_token_<?= $data['id']; ?>" ><?php echo $data['cryptocoin_first']['short_name']; ?> </td>
								<td class="sold_token_<?= $data['id']; ?>" ><?php echo $data['cryptocoin_second']['short_name']; ?> </td>
								<td class="sold_token_<?= $data['id']; ?>" ><?php echo $data['max_min_current_price_percent']; ?> </td>
								<td class="btc_value_<?= $data['id']; ?>"><?php echo __($statusArr[$data['status']]); ?> </td>
								<td class=" last">
									<a href="<?php echo $this->Url->build(['controller'=>'Coin','action'=>'coinpairChangestatus',$data['id']]) ?>" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> <?= __($buttonText); ?> </a>
									<a href="<?php echo $this->Url->build(['controller'=>'Coin','action'=>'coinpairEdit',$data['id']]) ?>" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> <?= __('Edit'); ?> </a>
	...
	function delete_section(id){
		bootbox.confirm("<?= __('Are you sure?'); ?>", function(result) {
			if(result == true){
				jQuery.ajax({ 
					//url: 'delete',
					url: '<?php echo $this->Url->build(['controller'=>'Coin','action'=>'coinpairDeleteProgram']); ?>',
