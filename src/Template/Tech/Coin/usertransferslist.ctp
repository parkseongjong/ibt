<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1> <a href="<?php echo $this->Url->build(['controller'=>'coin','action'=>'usertransferslist']);  ?>"><?= __('Send Rewards List') ?></a> </h1>
        <ol class="breadcrumb">
            <li><a href="<?php echo $this->Url->build(['controller'=>'pages','action'=>'dashboard']);  ?>"><i class="fa fa-dashboard"></i> <?= __('Home') ?></a></li>
            <li class="active"> <a href="<?php echo $this->Url->build(['controller'=>'coin','action'=>'usertransferslist']);  ?>"><?= __('Send Rewards List') ?> </a></li>
        </ol>
    </section>
                    ...
                    <form id="frm" method="get" class="form-horizontal form-label-left input_mask">
                        <div class="form-group">
                            ...
                            <div class="col-md-4 col-sm-1 col-xs-12">
                                <?php  echo $this->Form->input('',array('id'=>'total_main_trading_amount','placeholder'=>__('Total amount transferred from Main to Trading account'),'class' => 'form-control col-md-7 col-xs-12 has-feedback-left','label' =>false,'type'=>'text','value'=>__('Total Main Account → Trading Account Amount: ').number_format($mainToTradingSumShow,2),'readonly'=>'readonly')); ?>
                            </div>
                            <div class="col-md-4 col-sm-1 col-xs-12">
                                <?php  echo $this->Form->input('',array('id'=>'total_trading_main_amount','placeholder'=>__('Total amount transferred from Trading to Main account'),'class' => 'form-control col-md-7 col-xs-12 has-feedback-left','label' =>false,'type'=>'text','value'=>__('Total Trading Account → Main Account Amount: ').number_format($tradingToMainSumShow,2),'readonly'=>'readonly')); ?>
                            </div>
                            <div class="col-md-4 col-sm-1 col-xs-12">
                                <?php  echo $this->Form->input('',array('id'=>'total_withdrawals','placeholder'=>__('Total amount withdrawn'),'class' => 'form-control col-md-7 col-xs-12 has-feedback-left','label' =>false,'type'=>'text','value'=>__('Total Wallet ← Main Account Amount: ').number_format($withdrawSumShow,2),'readonly'=>'readonly')); ?>
                            </div>
                        </div>
                        <!-- Token Button List -->
                    ...
                    <div id="transferHistory" class="mt10 table-responsive">
                        <table class="two-axis table" id="historyData">
                            ...
                            foreach($listing->toArray() as $k=>$data){
								$this->add_system_log(200, $data['user_id'], 1, '고객 이체 기록 (Account transfers by Users) 조회 (이름, 전화번호)');
                                if($k%2==0) $class="odd";
                                else $class="even";
                        ...
                        $this->Paginator->options(array('url' => array('controller' => 'coin', 'action' => 'usertransferslist')+$searchArr));
