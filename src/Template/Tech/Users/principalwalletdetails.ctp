<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1> <a href="<?php echo $this->Url->build(['controller'=>'users','action'=>'principalwalletdetails']);  ?>"><?=__("Users Principal Wallet Details");?></a></h1>
        <ol class="breadcrumb">
            <li><a href="<?php echo $this->Url->build(['controller'=>'pages','action'=>'dashboard']);  ?>"><i class="fa fa-dashboard"></i> <?=__("Home");?></a></li>
            <li class="active"> <a href="<?php echo $this->Url->build(['controller'=>'users','action'=>'principalwalletdetails']);  ?>"><?=__("Users Principal Wallet Details");?></a></li>
        </ol>
    </section>
    <!-- Main content -->
                        ...
                        <table class="two-axis table" id="historyData">
                            ...
                            <?php
								foreach($listing->toArray() as $k=>$data){
									$this->add_system_log(200, $data['user_id'], 1, '고객 지갑 로그 조회 (이름, 전화번호, 지갑주소)');
									if($k%2==0) $class="odd";
									else $class="even";
									?>
									<tr class="<?=$class?>">
                                    ...
                                    <td><?= $this->get_transfer_type($data['type']); ?> </td>
                                    <td><?= $this->get_transfer_remark($data['remark']);?></td>
                            ...
                            <?php  if(count($listing->toArray()) < 1) {
                                echo "<tr class='even'><td colspan = '14' style='text-align: center;'>".__('No record found')."</td></tr>";
                            } ?>
                            </tbody>
                        </table>
                        <?php
							...
							$this->Paginator->options(array('url' => array('controller' => 'users', 'action' => 'principalwalletdetails')+$searchArr));
