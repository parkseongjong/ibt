<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1><a href="<?php echo $this->Url->build(['controller'=>'exchange','action'=>'buy-list']);  ?>"> <?= __('Buy List');?> </h1>
        <ol class="breadcrumb">
            <li><a href="<?php echo $this->Url->build(['controller'=>'pages','action'=>'dashboard']);  ?>"><i class="fa fa-dashboard"></i> <?= __('Home');?></a></li>
            <li class="active"><a href="<?php echo $this->Url->build(['controller'=>'exchange','action'=>'buy-list']);  ?>"> <?= __('Buy List');?></a></li>
        </ol>
    </section>
<br/>
                ...
                <div class="w3l-table-info agile_info_shadow">
                    ...
                    <form method="get" id="frm">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-1 col-sm-1 col-xs-12">
                                <?php  echo $this->Form->input('',array('id'=>'total_amount','placeholder'=>__('Total bought amount'),'class' => 'form-control col-md-7 col-xs-12 has-feedback-left','label' =>false,'type'=>'text','value'=>number_format(!empty($totalBuyAmount) ? $totalBuyAmount : 0 ,2),'readonly'=>'readonly')); ?>
                            </div>
                        ...
                        <div class="token-button-list-container"> <?= __('Status: ') ?>
                            <?php 
								foreach($statusList as $k=>$data) { ?>
									<button type="button" onclick="form_submit('status','<?=$data['DISTINCT BuyExchange']['status'];?>')"  class="btn btn-tokenss m-r-2 <?php if($this->request->query('status') ==  $data['DISTINCT BuyExchange']['status']) {echo 'selected';}?>"><?= __($data['DISTINCT BuyExchange']['status']);?></button>
                            <?php } ?>
                        </div>
                        ...
                        <table id="table-two-axis" class="two-axis table">
                            ...
                            $count= $serial_num;
							 foreach($listing->toArray() as $k=>$data){
								$this->add_system_log(200, $data['user']['id'], 1, '구매 목록 조회 (이름, 전화번호)');
								if($k%2==0) $class="odd";
								else $class="even";
							?>
								<tr class="<?=$class?>">
									...
									<td><?= number_format((float)$data['total_buy_spend_amount'],4)?> </td>
									<td><?= number_format((float)$data['buy_spend_amount'],4)?> </td>
									<td><?= $data['spendcryptocoin']['short_name']?> </td>
									<td><?= number_format((float)$data['total_buy_get_amount'],4)?> </td>
									<td><?= number_format((float)$data['buy_get_amount'],4)?> </td>
									...
									<td>
										...
										?>
											<button type="button" onclick="deleteOrder(<?=$data['id'];?>,'buy',<?= $data['user']['id']; ?>,this)" class="btn btn-primary btn-xs">삭제</button>
										<?php }?>
									</td>
