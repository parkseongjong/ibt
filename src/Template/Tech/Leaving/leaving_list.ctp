<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1><a href="<?php echo $this->Url->build(['controller'=>'Leaving','action'=>'leavingList']);  ?>"> <?= __('Leaving Users List');?></a> </h1>
        <ol class="breadcrumb">
            <li><a href="<?php echo $this->Url->build(['controller'=>'pages','action'=>'dashboard']);  ?>"><i class="fa fa-dashboard"></i> <?= __('Home');?></a></li>
            <li class="active"><a href="<?php echo $this->Url->build(['controller'=>'Leaving','action'=>'leavingList']);  ?>"> <?= __('Leaving Users List');?></a></li>
        </ol>
    </section>
    <!-- Main content -->
            ...
            <div class="w3l-table-info agile_info_shadow">
                ...
                <div class="table-responsive m-t-20">
                    <table id="table-two-axis" class="two-axis table" >
                        <thead>
							<tr style="text-align: center;vertical-align: middle">
								...
								<th  style="text-align: center;vertical-align: middle"><?= __('Leaving Date'); ?></th>
								<th  style="text-align: center;vertical-align: middle"><?= __('자산포기각서'); ?></th>
							</tr>
                        </thead>
                        <tbody>
                        <?php
                        foreach($leaving_list as $l){
							$this->add_system_log(200, $l->id, 1, '탈퇴 계정 고객 목록 조회');
                        ?>
						<tr  style="text-align: center;vertical-align: middle">
                            <td><?=$l->id;?></td>
                            <td><a href="javascript:void(0)" onclick="unmasking(this,'LN',<?= $l->id; ?>)" class="text-dark"><?= $this->masking('N',$l->name); ?></a></td>
							<!--<td><a href="javascript:void(0)" onclick="unmasking(this,'LE',<?/*= $l->id; */?>)" class="text-dark"><?/*= $this->masking('E',$l->email); */?></a></td>-->
							<!--<td><a href="javascript:void(0)" onclick="unmasking(this,'LP',<?/*= $l->id; */?>)" class="text-dark"><?/*= $this->masking('P',$l->phone_number); */?></a></td>-->
                            <td><?php echo substr($l->phone_number,-4,4) ?></td>
							<td><?= $l->last_login != null ? $l->last_login->format('Y-m-d H:i:s') : ''; ?></td>
							<td><?= $l->created != null ? $l->created->format('Y-m-d H:i:s') : ''; ?></td>
							<td><?= $l->annual_membership; ?></td>
							<td><?= $l->leave_date != null ? $l->leave_date->format('Y-m-d H:i:s') : ''; ?></td>
							<td>자산포기각서 여부</td>
                        </tr>	
                            <?php }  ?>
                        <?php  if(count($leaving_list) < 1) {
                    ...
                    <?php 
						...
						$this->Paginator->options(array('url' => array('controller' => 'Leaving', 'action' => 'leavingList')+$searchArr));
