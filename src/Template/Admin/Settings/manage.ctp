				  ...
				  <?php echo $this->Form->create($Pages,array('class'=>'form-horizontal form-label-left','novalidate','method'=>'post','enctype'=>'multipart/form-data'));?>
					...
					<?= $this->Flash->render() ?>
					  ...
					  <div class = "cms-details" style = "display:none">
						...
						<a href = "<?php echo $this->Url->build(['controller'=>'Settings' , 'action'=>'manage']);  ?>" class = "bck-btn" ><button class="btn btn-dark" type="button">Back</button></a>
...
jQuery(document).ready(function(){
	...
	jQuery('.cms_pages').change(function(){
		...
		var cms_id = jQuery(this).val();
			...
			jQuery.ajax({ 
						url: '<?php echo $this->Url->build(['controller'=>'Settings' , 'action'=>'search']);  ?>',
						data: {'cms_id':cms_id},
						type: 'POST',
