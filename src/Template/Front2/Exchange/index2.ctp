			...
			success : function(resp){
				...
				if($.isEmptyObject(resp.myBuyOrderList)){
					...
					$.each(resp.myBuyOrderList,function(key,value){
						...
						html = html + '<td>'+numberWithCommas(parseFloat(showAmount).toFixed(2))+'</td>';
						html = html + '<td>'+numberWithCommas(parseFloat((parseFloat(value.per_price).toFixed(2))*(parseFloat(showAmount).toFixed(2))).toFixed(2))+'</td>';
						if(status === 'Completed') {
				...
				if($.isEmptyObject(resp.mySellOrderList)){
					...
					$.each(resp.mySellOrderList,function(key,value){
						...
						html = html + '<td>'+getdateTime+'</td>';
						html = html + '<td>'+numberWithCommas(parseFloat(value.per_price).toFixed(2))+'</td>';
						html = html + '<td>'+numberWithCommas(parseFloat(parseFloat(showAmount).toFixed(2)/parseFloat(value.per_price).toFixed(2)).toFixed(2))+'</td>';
						html = html + '<td>'+numberWithCommas(parseFloat(showAmount).toFixed(2))+'</td>';
                        if(status === 'Completed') {
                            html = html + '<td>' + '<?= __('Completed')?>' + '</td>';
                        } else{
