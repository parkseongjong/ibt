			...
			success : function(resp){
				...
				if($.isEmptyObject(resp)){
					...
					/* 	if(value.get_cryptocoin_id==<?php echo $secondCoinId; ?>){
						...
						html = html + '<td class="right">'+parseFloat(totalPrice).toFixed(2)+'</td>';
						html = html + '</tr>';
					});
				}
		...
		$.ajax({
			...
			success : function(resp){
				if($.isEmptyObject(resp.current_price)){
					...
					$("#current_price_<?php echo $secondCoin."_".$firstCoin; ?>").html(returnPrice);
			...
			success : function(resp){
				...
				if($.isEmptyObject(resp.buyOrderList)){
					...
					$.each(resp.buyOrderList,function(key,value){
							...
							html = html + '<td class="fill_amount">'+parseFloat(value.sum).toFixed(2)+'</td>';
							html = html + '<td class="right">'+parseFloat(parseFloat(value.per_price)*parseFloat(value.sum)).toFixed(2)+'</td>';
							html = html + '</tr>';
