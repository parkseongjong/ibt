...
<div class="container">
	<div class="custom_frame document">
		...
		<div class="contents">
			<ul>
				<li class="title"><?= __('terms-rental') ?></li>
			</ul>
			...
			<div class="prev-privacy-area">
				<ul>
					<li><?= __('terms-rental') ?> : 2021.03.01 <a href="javascript:void(0)" onclick="getTerms('2021-03-01')"><?=__('Show2');?></a></li>
				</ul>
			</div>
		</div>
		<div class="cls"></div>
...
<script>
	function getTerms(file_name='2021-03-01'){
		const md_file = '<zero-md src="/dev/terms_rental/'+file_name+'.md" class="text_md_file" ><template><style>.markdown-body { font-family: "Noto Sans KR", "Montserrat", sans-serif !important; }</style></template></zero-md>';
		$('#text_area').html('');
