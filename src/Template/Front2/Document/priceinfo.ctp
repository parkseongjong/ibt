	...
	<div class="custom_frame document">
		...
		<div class="contents">
            ...
            <div class="tp3-ctc-coupon-area">
                <ul class="nn_title2">
                    <li> When trading digital assets in COIN IBT, the same commission rate (%) is applied to all digital assets. </li>
                    <li>Withdrawal fees may change according to exchange policies and market conditions. We will inform you through advance notice. </li>
                </ul>
                <div class="coupon-purchase-area" id="lvl3d">
                        ...
                        <thead>
							...
							?>
									<div id="ifDeposit<?=$l['short_name'];?>" class="flex-child" style="transform: translateX(70%);display:none;">
										<span style="color: red;"><?=$l['short_name'];?>is not enough</span>
									</div>
							<?php 
									}
								}
					...
					?>
							<tr>
                                <td style="width: 25%;">
                                    <div><?=$l['short_name'];?>coupon <?= number_format($l['amount']); ?></div>
                                    ...
                                    <?php 
										...
										if($none != 1){ //  로그인 되어 있을 경우
											...
											if($lastTime >= 0 && $lastTime <= $hourToMin){ // 최근 구매 시간 체크
												...
												if($lastTimeMin <= 60){
													if($lastTimeMin == 0){
														$btnText = 'open after a while';
													} else {
														// $btnText = $lastTimeMin.' 분 후 이용 가능';
														$btnText = 'Available in '.$lastTimeMin.' minutes';
													}
												} else {
													$m1 = gmdate('H',$lastTimeMin*60);
													$m2 = gmdate('i',$lastTimeMin*60);
													$btnText = $m1.'hour '.$m2.' minutes';
												}
												$disabled = 'disabled';
											}
										}
