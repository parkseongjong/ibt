...
<div class="content-container">
	<div class="content-inner">
		<div class="caution-container">
			<div class="caution-title"> Please check the notes below before you withdraw. </div>
			<div class="caution-list-wrap">
      <ul class="caution-list">
        <li>If a user withdraws from the service, the company keeps personal information for one year to prevent illegal use.</li>
        <li>In this case, personal information is kept in a non-identifiable state and cannot be re-registered with the same ID.</li>
        <li>Payment processing services are provided when assets remain in accordance with Article 6 of the Consumer Protection Act in Electronic Commerce, etc. and Article 5-4 of the Act on the Reporting and Use of Specific Financial Transaction Information. The withdrawal process is completed after writing the asset renunciation memorandum and handwritten signature.</li>
      </ul>
			</div>
			<div class="caution-check-wrap">
				<input type="checkbox" id="agree" class="caution-checkbox" onchange="check_agree()">
				<label for="agree" class="caution-checkbox-label"> I have read all the cautions and agree to them. </label>
        <p id="error_msg1" style="display:none;">Agree to the cancellation notice</p>
			</div>
		</div>
		<div class="form-container">
			<div class="form-top-area">
				<div class="user-id-form">
					<div class="user-label user-id-label">ID</div>
					<div id="user-id" class="user-id-value"><?=$username;?></div>
				</div>
				<div class="user-password-form">
					<div class="user-label user-password-label">Confirm Password</div>
					<input type="password" id="user-password" name="password" class="user-password-input" onkeyup="check_password(this.value)" placeholder="Input Password" /> </div>
				<div class="warn-area" id="error_msg2" style="display:none;"> Please enter a password. </div>
			</div>
			<?= $this->Flash->render(); ?>
			<button type="button" class="form-button" onClick="onClickDeleteAccountButton()"> Delete Account </button>
		</div>
	</div>
</div>
