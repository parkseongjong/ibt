				...
				<tbody>
					<tr>
						<td rowspan="2" style="width:46%; padding-bottom:20px">
							...
							<div class="total_amount"> <?= number_format((float)$totalKRWBalance, 2) ?>
<!--								 <span class="unit">--><?//=__('KRW') ?><!--</span>-->
		...
		<ul class="tab_menu">
			<li id="index" class="on"><a href="<?php echo $this->Url->build(['controller'=>'wallet','action'=>'index']) ?>"><?=__('Transaction Details') ?></a></li>
			<li id="history"><a href="<?php echo $this->Url->build(['controller'=>'wallet','action'=>'history']) ?>"><?=__('Deposit Details') ?></a></li>
		</ul>
		<div class="filter">
			<ul>
				<li>
...
<script>
    ...
    function transferHistory() {
        $.ajax({
            ...
            url : '<?php echo $this->Url->Build(['controller'=>'wallet','action'=>'transferHistory']) ?>',
            type : 'get',
            dataType : 'json',
            success : function(resp){
    ...
    function selectedVal(slct,type){
        ...
        var method;
        if(slct === '0'){
            urlSelected = '<?= $this->Url->Build(['controller'=>'wallet','action'=>'transferHistory']) ?>';
            method = 'get';

        } else {
            if(type === 0){
                urlSelected = '<?= $this->Url->build(['controller' => 'wallet', 'action' => 'transferHistoryHours']); ?>/' + slct;
                method = 'post';
            }
            else {
                urlSelected = '<?= $this->Url->build(['controller' => 'wallet', 'action' => 'transferHistoryCoin']); ?>/' + slct;
                method = 'post';
            }
        }
    ...
    function showSelected(start,end) {
        $.ajax({
            url : "<?= $this->Url->Build(['controller'=>'wallet','action'=>'transferHistoryCalendar']) ?>/" + start +"/"+ end,
			beforeSend: function(xhr){
				xhr.setRequestHeader('X-CSRF-Token', $("input[name='_csrfToken']").val());
			},
            type : 'post',
