...
if (Configure::read('debug')):
    ...
    $this->assign('templateName', 'error520.ctp');
...
<?php if (Configure::read('debug')){?>
    <h2><?= __d('cake', 'Web Server Returned an Unknown Error') ?></h2>
    <p class="error">
        <strong><?= __d('cake', 'Web Server Returned an Unknown Error') ?>: </strong>
        <?= h($message) ?>
    </p>
<?php  } else{ ?>
    ...
    <div class="content-container">
        <div class="content-inner">
            ...
            <div class="p404-subtitle">
                <?= __('Web Server Returned an Unknown Error');?>
            </div>
            <div class="p404-subtitle2">
                <div><?= __('Internal server error due to empty response');?></div>
            </div>
