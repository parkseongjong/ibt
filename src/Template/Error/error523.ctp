...
if (Configure::read('debug')):
    ...
    $this->assign('templateName', 'error523.ctp');
...
<?php if (Configure::read('debug')){?>
    <h2><?= __d('cake', 'Server is unreachable') ?></h2>
    <p class="error">
        <strong><?= __d('cake', 'Server is unreachable Error') ?>: </strong>
        <?= h($message) ?>
    </p>
<?php  } else{ ?>
    ...
    <div class="content-container">
        <div class="content-inner">
            ...
            <div class="p404-subtitle">
                <?= __('Server is Unreachable Error');?>
            </div>
            <div class="p404-subtitle2">
                <div><?= __('Please check your internet connection');?></div>
            </div>
