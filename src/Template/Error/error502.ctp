...
if (Configure::read('debug')):
    ...
    $this->assign('templateName', 'error502.ctp');
...
<?php if (Configure::read('debug')){?>
    <h2><?= __d('cake', 'Bad Gateway') ?></h2>
    <p class="error">
        <strong><?= __d('cake', 'Bad Gateway Error') ?>: </strong>
        <?= h($message) ?>
    </p>
<?php  } else{ ?>
    ...
    <div class="content-container">
        <div class="content-inner">
            ...
            <div class="p404-subtitle">
                <?= __('Bad Gateway Error');?>
            </div>
            <div class="p404-subtitle2">
                <div><?= __('Invalid response received');?></div>
                <div><?= __('Are you sure that you entered the correct URL?');?></div>
            </div>
