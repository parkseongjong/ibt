...
if (Configure::read('debug')):
    ...
    $this->assign('templateName', 'error410.ctp');
...
<?php if (Configure::read('debug')){?>
    ...
    <p class="error">
        ...
        <?= sprintf(
            __d('cake', 'Requested page is no longer available.'),
            "<strong>'{$url}'</strong>"
        ) ?>
    </p>
<?php  } else{ ?>
    ...
    <div class="content-container">
        <div class="content-inner">
            ...
            <div class="p404-subtitle">
                <?= __('Request no longer available Error');?>
                <!-- EN: Sorry, we were unable to find that page -->
            </div>
            <!-- EN에선 p404-subtitle2 제거 -->
            <div class="p404-subtitle2">
                <div><?= __('Sorry! The requested page is no longer available.');?></div>
            </div>
