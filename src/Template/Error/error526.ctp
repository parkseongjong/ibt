...
if (Configure::read('debug')):
    ...
    $this->assign('templateName', 'error526.ctp');
...
<?php if (Configure::read('debug')){?>
    <h2><?= __d('cake', 'Invalid SSL Certificate') ?></h2>
    <p class="error">
        <strong><?= __d('cake', 'Invalid SSL Certificate Error') ?>: </strong>
        <?= h($message) ?>
    </p>
<?php  } else{ ?>
    ...
    <div class="content-container">
        <div class="content-inner">
            ...
            <div class="p404-subtitle">
                <?= __('Invalid SSL Certificate Error');?>
            </div>
            <div class="p404-subtitle2">
                <div><?= __('Warning! SSL Certificate is not verified.');?></div>
            </div>
