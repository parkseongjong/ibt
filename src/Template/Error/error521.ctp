...
if (Configure::read('debug')):
    ...
    $this->assign('templateName', 'error521.ctp');
...
<?php if (Configure::read('debug')){?>
    <h2><?= __d('cake', 'Web Server is Down') ?></h2>
    <p class="error">
        <strong><?= __d('cake', 'Web Server is Down Error') ?>: </strong>
        <?= h($message) ?>
    </p>
<?php  } else{ ?>
    ...
    <div class="content-container">
        <div class="content-inner">
            ...
            <div class="p404-subtitle">
                <?= __('Web Server is Down Error');?>
            </div>
            <div class="p404-subtitle2">
                <div><?= __('Server refused connections.');?></div>
                <div><?= __('You are not authorized to access this page. Please contact us.');?></div>
            </div>
