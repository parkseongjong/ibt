...
if (Configure::read('debug')):
    ...
    $this->assign('templateName', 'error522.ctp');
...
<?php if (Configure::read('debug')){?>
    <h2><?= __d('cake', 'Connection Timed Out') ?></h2>
    <p class="error">
        <strong><?= __d('cake', 'Connection Timed Out Error') ?>: </strong>
        <?= h($message) ?>
    </p>
<?php  } else{ ?>
    ...
    <div class="content-container">
        <div class="content-inner">
            ...
            <div class="p404-subtitle">
                <?= __('Connection Timed Out Error');?>
            </div>
            <div class="p404-subtitle2">
                <div><?= __('Please check your internet connection or try later');?></div>
            </div>
