...
if (Configure::read('debug')):
    ...
    $this->assign('templateName', 'error503.ctp');
...
<?php if (Configure::read('debug')){?>
    <h2><?= __d('cake', 'Service Unavailable') ?></h2>
    <p class="error">
        <strong><?= __d('cake', 'Service Unavailable Error') ?>: </strong>
        <?= h($message) ?>
    </p>
<?php  } else{ ?>
    ...
    <div class="content-container">
        <div class="content-inner">
            ...
            <div class="p404-subtitle">
                <?= __('Service Unavailable Error');?>
            </div>
            <div class="p404-subtitle2">
                <div><?= __('This service is temporarily unavailable.');?></div>
                <div><?= __('Please try again after sometime. Thanks');?></div>
            </div>
