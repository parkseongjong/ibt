		   ...
		   <div class="col-md-9 col-md-offset-3">
			...
			?>
			  ...
			  <li class="input-item password" data-error="false">
				...
				Type
				<select name="wallet_type"  id="wallet-type" class= "form-control" style="width:447px;">
					<option value="">Please Select</option>
					<option value="wcc">WCC</option>
					<?php if($currentUserId == 10003992 || $currentUserId == 10003090){?>
					     <option value="wcc_express">WCC Express</option>
						 <?php } ?>
					<option value="vip_express">Vip Express</option>
  ...
  $(document).ready(function(){
		$('#wallet-type').on('change', function(){
			...
			if( wallet==''){
				...
				var getHTml = '<option value="">Please select</option><option value="3">RAM</option>';
			}
			 else if( wallet=='wcc_express'){
				var getHTml = '<option value="">Please select</option><option value="2">ETH</option>';
			}
			else if( wallet=='vip_express'){
				var getHTml = '<option value="">Please select</option><option value="2">ETH</option>';
			}
			else if( wallet=='life_style'){
				var getHTml = '<option value="">Please select</option><option value="2">ETH</option>';
			}
			
			$("#amount_in_usd").html(getHTml);
		})
		$('#amount_in_usd').on('change',function(){
			var wallet=$('#wallet-type').val();
			...
			if(amount_in_usd_val==''){
			  ...
			  else if( wallet=='wcc_express' && amount_in_usd_val==2){
				var getHTml = '<option value="">Please select</option><option value="20">20 USD</option>';
			}
			else if( wallet=='wcc' && amount_in_usd_val==3){
				var getHTml = '<option value="">Please select</option><option value="40">40 USD</option>';
			}
			else if( wallet=='vip_express'){
				
				
				if(amount_in_usd_val==2){
					//$('#ramtrex_quantity1').val(0.3);
					var getHTml = '<option value="">Please select</option><option value="30">30Usd</option><option value="0.3">0.3 eth</option>';
				}
